...
echo adding server
openstack  server create --flavor 5 --image xenial --nic net-id=private --key-name id_rsa test --insecure
...
sleep 15
openstack  server add floating ip test ${floating_ip} --insecure
